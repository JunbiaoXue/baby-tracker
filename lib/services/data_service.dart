...
class DataService extends ChangeNotifier {
  ...
  // ---- 合并时间相近的记录 ----
  // 将指定时间内的事件合并为一组
  static const int mergeIntervalMinutes = 10; // 10分钟内的记录视为一组
  ...
  List<Map<String, dynamic>> getMergedRecords() {
    ...
    for (final item in allRecords) {
      if (merged.isEmpty) {
        ...
        final lastTime = last['time'] as DateTime;
        final currentTime = item['time'] as DateTime;
        final diff = currentTime.difference(lastTime).inMinutes;
        
        if (diff <= mergeIntervalMinutes) {
          // 合并到上一组
          if (last['events'] == null) {
            last['events'] = [last];
          }
  ...
  Map<String, dynamic> getFrequencyStats() {
    ...
    for (int i = 0; i < 7; i++) {
      ...
      dailyStats[key] = {'feeding': 0, 'diaper': 0};
    }
    ...
    for (final f in _feedingRecords) {
      if (f.time.isAfter(weekStart)) {
        ...
        if (dailyStats.containsKey(key)) {
          dailyStats[key]!['feeding'] = dailyStats[key]!['feeding']! + 1;
        }
      }
    }
    ...
    for (final d in _diaperRecords) {
      if (d.time.isAfter(weekStart)) {
        ...
        if (dailyStats.containsKey(key)) {
          dailyStats[key]!['diaper'] = dailyStats[key]!['diaper']! + 1;
        }
      }
    }
